...
class DataLoader:
    ...
    def load_data(self):
        ...
        self.movies_df['genres'] = self.movies_df['genre'].str.split(', ')  # Changed line
        self.movies_df['sid'] = range(len(self.movies_df))

    def process_data(self):
        exploded_genres = self.movies_df.explode('genres')  # Changed line
        exploded_genres.dropna(subset=['genres'], inplace=True)  # Changed line
        self.final_df = exploded_genres[['sid', 'names', 'genres', 'overview']]  # Changed line
        self.final_df.drop_duplicates(inplace=True)

        unique_genres = self.final_df['genres'].unique()  # Changed line
        genre_to_id = {genre: idx + 1 for idx, genre in enumerate(unique_genres)}
        self.final_df['genre_id'] = self.final_df['genres'].map(genre_to_id)  # Changed line
    ...
    def analyze_genre_counts(self):
        genre_counts = self.final_df['genres'].value_counts()  # Changed line
    ...
    def analyze_action_genre(self):
        ...
        action_df['is_action'] = action_df['genres'] == 'Action'  # Changed line
...
#%%
# Usage
data_loader = DataLoader('data/imdb_movies.csv')
